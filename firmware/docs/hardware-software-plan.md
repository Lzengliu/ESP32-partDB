...
摄像头与 TF 卡使用开发板资料中的 Freenove ESP32-S3 EYE 示例引脚。ILI9488 不使用并口，只使用 4 线 SPI；否则在保留摄像头时 GPIO 不足。FT6336 和 PN532 默认共用独立 I2C 总线，地址分别为 `0x38` 和 `0x24`，若实际模块上拉或电平冲突，再把 PN532 改到软件 I2C。
...
| CAM SCCB SDA/SCL | 4 / 5 | 摄像头专用，不和触摸/NFC 共用 |
...
| Touch/NFC SDA/SCL | 35 / 36 | 独立 I2C 总线 |
| Touch RST/INT | 37 / 41 | FT6336 |
...
| 摄像头 | `camera_mgr.*` | `esp_camera` 初始化、抓图、释放资源 |
...
2. 外设联调：ILI9488 清屏、FT6336 坐标、PN532 UID、TF 挂载、摄像头抓图。
