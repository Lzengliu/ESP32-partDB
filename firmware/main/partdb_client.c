...
static esp_err_t build_url(const char *path, char *out, size_t out_len)
{
    ...
    if (strncmp(endpoint, "/api/", 5) == 0) {
        ...
        snprintf(normalized, sizeof(normalized), "/api%s%s", slash, endpoint);
        snprintf(out, out_len, "%s%s", base, normalized);
        return ESP_OK;
    }

    const char *slash = endpoint[0] == '/' || endpoint[0] == '\0' ? "" : "/";
    snprintf(out, out_len, "%s%s%s", base, slash, endpoint);
    return ESP_OK;
}
                                     ...
                                     char *out, size_t out_len, int *http_status)
{
    ...
    char url[APP_CONFIG_URL_LEN + 384];
...
esp_err_t partdb_client_get_json(const char *path, char *out, size_t out_len, int *http_status)
{
    ...
    esp_err_t err = partdb_client_request_json(PARTDB_HTTP_GET, path, NULL, NULL,
