...
| 二维码识别 | C/C++，ZXing-C++ | 摄像头取图后在 ESP32 本地解码二维码 |
...
触摸通过软件 I2C 读取，当前原型使用 `GPIO3/GPIO44(RX)`，`GPIO46` 为复位。
...
NFC 使用 PN532 模块，当前使用独立硬件 I2C：`SDA=GPIO43(TX)`、`SCL=GPIO41`。FT6336 触摸单独使用软件 I2C `GPIO3/GPIO44(RX)`，`GPIO46` 为触摸复位，当前原型不接 `INT`。项目需求是 NFC 与摄像头不共用硬件接口：摄像头 SCCB 保持在 `GPIO4/GPIO5`，NFC 默认随系统启动保持在线。

当前固件会在 `/api/status.hardware.i2c` 中报告 `nfc_shares_touch_i2c` 和 `nfc_shares_camera_sccb`，用于确认接线是否符合预期。
...
V1.0 遗留问题：当前 PN532 在实机上仍可能显示 `ESP_ERR_NOT_FOUND`，需要继续验证 PN532 I2C 模式拨码、接线和上电时序。
...
V1.0 物理按键逻辑已预留，当前 Freenove 原型没有配置 GPIO。保留相机、屏幕、触摸、NFC、TF 卡和 USB 后，没有 4 个无冲突外部 IO 可直接使用；下一版 PCB 需要单独预留上、下、确定/返回、息屏/唤醒四个非绑带、非 MSPI/PSRAM、非 USB 的 GPIO。
...
| SPI MISO | GPIO48 |
| SPI CS | GPIO43 |
| SPI 频率 | 100 kHz（当前联调频率） |
| SPI 总线 | 与 ILI9488 共享 SCLK/MOSI/MISO，独立 CS；不与摄像头 SCCB 共用 |
